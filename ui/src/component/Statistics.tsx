...
function lines(
  ...
  switch (viewType) {
    case ViewType.TOTAL_BALANCE:
      const totalColorPalette = generateColorPalette(
        1 + Object.values(TotalType).length,
      )
      return (
        <React.Fragment>
          {Object.values(TotalType).map((totalType, index) => (
            <Line
              key={totalType}
              type="monotone"
              dataKey={totalType}
    ...
    case ViewType.TOTAL_TRANSFER:
      const totalsTransferColorPalette = generateColorPalette(
        Object.values(TotalType).length,
      )
      return (
        <React.Fragment>
          {Object.values(TotalType).map((totalType, index) => (
            <Line
              ...
              stroke={dull(
                totalType,
                shownLines,
                totalsTransferColorPalette[index],
              )}
              name={titleCase(totalType)}
            />
          ))}
    ...
    case ViewType.ACCOUNT_BALANCE:
      const accountColorPalette = generateColorPalette(
    ...
    case ViewType.ACCOUNT_TRANSFER:
      const accountTransfersColorPalette = generateColorPalette(
        Object.values(accounts).length,
      )
      return (
        <React.Fragment>
          {Object.values(accounts).map((account, index) => (
            <Line
              ...
              stroke={dull(
                account.id,
                shownLines,
                accountTransfersColorPalette[index],
              )}
              name={`${account.name} (${titleCase(account.type)})`}
            />
          ))}
    ...
    case ViewType.FLOW:
      const flowColorPalette = generateColorPalette(
        1 + Object.values(TotalType).length,
      )
      return (
        <React.Fragment>
    ...
    case ViewType.FLOW_GROUPING:
      const flowGoupingColorPalette = generateColorPalette(
        1 + Object.values(FlowGroupingType).length,
      )
      return (
        <React.Fragment>
          {Object.values(FlowGroupingType).map((flowGroupingType, index) => (
            <Line
              ...
              stroke={dull(
                flowGroupingType,
                shownLines,
                flowGoupingColorPalette[index + 1],
              )}
  ...
  useEffect(() => {
    const startDate =
      lineStartDate === ""
        ? statistics.length === 0
          ? new Date()
          : new Date(statistics[Math.max(0, statistics.length - 10)].date)
        : new Date(lineStartDate)
    const endDate =
      lineEndDate === ""
        ? statistics.length === 0
          ? new Date()
          : new Date(statistics[Math.max(statistics.length - 1)].date)
        : new Date(lineEndDate)
      ...
      <Row xs={1} md={1} xl={1}>
        <Col>
          <Form.Group>
            ...
            >
              {Object.keys(GraphType).map((type) => (
      ...
      >
        <Col>
          {graphType === GraphType.LINE ? (
            <LineChart
              width={(widthRef.current?.offsetWidth ?? 0) * 0.95}
              height={vh * 0.7}
            ...
            >
              ...
              <XAxis dataKey="date" />~
              <YAxis
                tickFormatter={(value: any) => formattedUnknownAmount(value)}
              />
              <Legend
                onClick={(e) => {
                  if (e.dataKey) {
