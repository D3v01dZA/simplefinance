        ...
        <Form>
          <Form.Group>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                setExpense({ ...expense, external: e.target.value })
              }
            ></Form.Control>
          </Form.Group>
          <Form.Group>
          ...
          <Form.Group>
            ...
            <Form.Control
              type="text"
              isInvalid={!isValueValid(expense?.value)}
              value={expense?.value}
              onChange={(e) =>
                setExpense({ ...expense, value: e.target.value })
              }
            ></Form.Control>
          </Form.Group>
        </Form>
  ...
  useEffect(() => {
    ...
    if (descriptionFilter !== "") {
      ...
      filtered = filtered.filter((expense) =>
        expense.description.toLowerCase().includes(use),
      )
    }
    if (externalFilter !== "") {
      let use = externalFilter.toLowerCase()
      filtered = filtered.filter((expense) =>
        expense.external.toLowerCase().includes(use),
      )
    }
    if (dateFilter !== "") {
      const filter = new Date(dateFilter + "T00:00:00").getTime()
      filtered = filtered.filter(
        (expense) => new Date(expense.date + "T00:00:00").getTime() === filter,
      )
    }
    if (lastNFilter !== "") {
      ...
      filtered = filtered.filter(
        (transaction) =>
          new Date(transaction.date + "T00:00:00").getTime() >= date.getTime(),
      )
    }
      ...
      <Popover.Body>
        ...
        <Form.Group>
          ...
          <Form.Control
            ...
            onChange={(e) => setSearchParams("external", e.target.value)}
          />
        </Form.Group>
        {clearFilters}
      </Popover.Body>
              ...
              <tr>
                <th>
                  ...
                  <OverlayTrigger
                    trigger="click"
                    placement="bottom"
                    overlay={categoryFilterPopover}
                  >
                    <FontAwesomeIcon
                      color={categoryFilter === "none" ? undefined : "blue"}
                      icon={faFilter}
                    />
                  </OverlayTrigger>
                </th>
                <th>
                  ...
                  <OverlayTrigger
                    trigger="click"
                    placement="bottom"
                    overlay={externalFilterPopover}
                  >
                    <FontAwesomeIcon
                      color={externalFilter === "" ? undefined : "blue"}
                      icon={faFilter}
                    />
                  </OverlayTrigger>
                </th>
                <th>
                  ...
                  >
                    <FontAwesomeIcon
                      color={descriptionFilter === "" ? undefined : "blue"}
                      icon={faFilter}
                    />
                  </OverlayTrigger>
                </th>
                <th>
                  ...
                  >
                    <FontAwesomeIcon
                      color={dateFilter === "" ? undefined : "blue"}
                      icon={faFilter}
                    />
                  </OverlayTrigger>
                </th>
                ...
                <tr key={index}>
                  ...
                  <td style={cellStyle("100px")}>
                    <Form.Control
                      ...
                      disabled={savingInlineExpense}
                      value={
                        inlineExpense.id === expense.id && inlineExpense.date
                          ? inlineExpense.date
                          : expense?.date
                      }
                      onChange={(e) =>
                  ...
                  <td style={cellStyle("100px")}>
                    <Form.Control
                      ...
                      isInvalid={
                        inlineExpense.id === expense.id && inlineExpense.value
                          ? !isValueValid(inlineExpense.value)
                          : !isValueValid(expense?.value)
                      }
                      value={
                        inlineExpense.id === expense.id && inlineExpense.value
                          ? inlineExpense.value
