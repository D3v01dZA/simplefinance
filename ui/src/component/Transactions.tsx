    ...
    <tr>
      ...
      <td style={cellStyle("100px")}>
        <Form.Control
          ...
          value={
            inlineTransaction.id === transaction.id &&
            inlineTransaction.description
              ? inlineTransaction.description
      ...
      <td style={cellStyle("100px")}>
        <Form.Control
          ...
          disabled={savingInlineTransaction}
          value={
            inlineTransaction.id === transaction.id && inlineTransaction.date
              ? inlineTransaction.date
              : transaction?.date
          }
          onChange={(e) =>
            setInlineTransaction({ id: transaction.id, date: e.target.value })
          }
          onBlur={(e) => {
            if (
              inlineTransaction.id === transaction.id &&
      ...
      <td style={cellStyle("100px")}>
        <Form.Control
          ...
          isInvalid={
            inlineTransaction.id === transaction.id && inlineTransaction.value
              ? !isValueValid(inlineTransaction.value)
              : !isValueValid(transaction?.value)
          }
          value={
            inlineTransaction.id === transaction.id && inlineTransaction.value
              ? inlineTransaction.value
              : transaction.value
          }
          onChange={(e) =>
            setInlineTransaction({ id: transaction.id, value: e.target.value })
          }
          onBlur={(e) => {
            if (
              inlineTransaction.id === transaction.id &&
      ...
      <td style={cellStyle("200px")}>
        <AccountName accountId={transaction.accountId} accounts={accounts} />
      </td>
      <td style={cellStyle("200px")}>
        <AccountName
...
export function Transactions() {
  ...
  const [lastTransactionByTypeFilter, setLastTransactionByTypeFilter] =
    useState<"none" | TransactionType>("none")
  ...
  const [inlineTransaction, setInlineTransaction] = useState<
    Partial<JTranscation>
  >({})
  const [savingInlineTransaction, setSavingInlineTransaction] =
    useState<boolean>(false)

  const [showAdding, setShowAdding] = useState(false)
  const [adding, setAdding] = useState(false)
  const [addingTransaction, setAddingTransaction] = useState<
    Partial<WorkingTransaction>
  >({})
  ...
  useEffect(() => {
    if (lastTransactionByTypeFilter !== "none") {
      setFilteredTransactions(
        filterTransactions(transactions, lastTransactionByTypeFilter),
      )
    } else {
      ...
      if (descriptionFilter !== "") {
        ...
        filtered = filtered.filter((transaction) =>
          description(transaction).toUpperCase().includes(filter),
        )
      }
      if (dateFilter !== "") {
        ...
        filtered = filtered.filter(
          (transaction) =>
            new Date(transaction.date + "T00:00:00").getTime() === filter,
        )
      }
      if (lastNFilter !== "") {
        ...
        filtered = filtered.filter(
          (transaction) =>
            new Date(transaction.date + "T00:00:00").getTime() >=
            date.getTime(),
        )
      }
  ...
  const globalFilters = (
    <React.Fragment>
      ...
      <Form.Group>
        ...
        <Form.Select
          value={lastTransactionByTypeFilter}
          onChange={(e) =>
            setSearchParams("lastTransactionByType", e.target.value)
          }
        >
      ...
      <Popover.Body>
        ...
        <Form.Group>
          ...
          <Form.Control
            ...
            onChange={(e) => setSearchParams("description", e.target.value)}
          />
        </Form.Group>
        {globalFilters}
        {clearFilters}
    ...
    <Popover>
      <Popover.Body>
        ...
        <Form.Group>
          ...
          <Form.Control
            disabled={globalFilterActive()}
            isInvalid={lastNFilter !== "" && !isValueValid(lastNFilter)}
            type="text"
            value={lastNFilter}
            onChange={(e) => setSearchParams("lastN", e.target.value)}
          />
          <Form.Select
            disabled={globalFilterActive()}
            value={lastSFilter}
            onChange={(e) => setSearchParams("lastS", e.target.value)}
          >
            <option value={"none"}></option>
            {Object.keys(LastSType).map((type) => (
              <option key={type} value={type}>
                {titleCase(type)}
              </option>
            ))}
  ...
  return (
    <Container>
              ...
              <tr>
                <th>
                  ...
                  <OverlayTrigger
                    trigger="click"
                    placement="bottom"
                    overlay={transactionTypeFilterPopover}
                  >
                    <FontAwesomeIcon
                      color={
                        globalFilterActive()
                ...
                <th>
                  ...
                  <OverlayTrigger
                    trigger="click"
                    placement="bottom"
                    overlay={descriptionFilterPopover}
                  >
                    <FontAwesomeIcon
                      color={
                        globalFilterActive()
            ...
            <tbody>
              {transactionsToDisplay().map((transaction) => (
                <Transaction
                  ...
                  setInlineTransaction={setInlineTransaction}
                  saveInlineTransaction={saveInlineTranscation}
                  savingInlineTransaction={savingInlineTransaction}
                />
              ))}
              <tr>
                <td></td>
                    ...
                    {accountId !== undefined ? null : (
                      <>
                        <Button
                          variant="primary"
                          onClick={() => {
                            setBulkAddingTransactions(
                              bulkTranscationsDefault({
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setAddingTransaction({
                          ...
                          description: "",
                          date: today(),
                          accountId: accountId ?? Object.keys(accounts)[0],
                        })
                        setShowAdding(true)
                      }}
                    >
      ...
      <TransactionModal
        accounts={accounts}
        settings={settings}
        singleAccount={accountId !== undefined}
        singleType={false}
        singleDate={false}
        show={showAdding}
        setShow={setShowAdding}
        transaction={addingTransaction}
        setTransaction={setAddingTransaction}
        saving={adding}
        save={() => {
          setAdding(true)
      ...
      <TransactionModal
        ...
        save={() => {
          ...
          post(
            server,
            `/api/transaction/${editingTransaction.id}/`,
            editingTransaction,
          )
            .then(() => refreshTransactions())
            .catch((error) => err(error))
