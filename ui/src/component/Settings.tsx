...
export function Settings() {
  ...
  const [savingRepeatingTransfers, setSavingRepeatingTransfers] =
    useState(false)
  ...
  function saveMultipleAccountBasedValue(
    ...
    if (toSave) {
      if (accounts === undefined || accounts.length === 0) {
        ...
        post(server, `/api/setting/${toSave.id}/`, {
          ...toSave,
          value: accounts.join(","),
        })
          .then(() => refreshSettings())
  ...
  function save() {
    if (updatedDefaultTransactionFromAccountId !== "") {
      ...
      let toSave: JSetting | undefined =
        settings[SettingKey.DEFAULT_TRANSACTION_FROM_ACCOUNT_ID]
      if (toSave) {
        post(server, `/api/setting/${toSave.id}/`, {
          ...toSave,
          value: updatedDefaultTransactionFromAccountId,
        })
          .then(() => refreshSettings())
          .catch((error) => err(error))
          .finally(() => {
        ...
        post(server, `/api/setting/`, {
          key: SettingKey.DEFAULT_TRANSACTION_FROM_ACCOUNT_ID,
          value: updatedDefaultTransactionFromAccountId,
        })
          .then(() => refreshSettings())
          .catch((error) => err(error))
    ...
    if (dirtyRepeatedTransfers) {
      ...
      if (toSave) {
        if (updatedRepeatingTransfers.length === 0) {
          ...
          post(server, `/api/setting/${toSave.id}/`, {
            ...toSave,
            value: JSON.stringify(updatedRepeatingTransfers),
          })
            .then(() => refreshSettings())
            .catch((error) => err(error))
            .finally(() => {
        ...
        post(server, `/api/setting/`, {
          key: SettingKey.REPEATING_TRANSFERS,
          value: JSON.stringify(updatedRepeatingTransfers),
        })
          .then(() => refreshSettings())
          .catch((error) => err(error))
  ...
  function updateRepeatedTransfer<K extends keyof RepeatingTransfer>(
    index: number,
    key: K,
    value: RepeatingTransfer[K],
  ) {
    setUpdatedRepeatingTransfers(
      updatedRepeatingTransfers.map((transfer, i) => {
        if (i === index) {
        ...
        <Col>
          <Form.Group>
            ...
            <Form.Select
              ...
              onChange={(e) =>
                setUpdatedDefaultTransactionFromAccountId(e.target.value)
              }
            >
          ...
          {updatedRepeatingTransfers.map((repeatingTransfer, index) => (
            <Row key={index} xs={1} md={2} xl={3}>
              <Col>
                <Form.Group>
                  <Form.Label>Start Date</Form.Label>
                  <Form.Control
                    type="date"
                    value={repeatingTransfer.start}
                    onChange={(e) =>
                      updateRepeatedTransfer(index, "start", e.target.value)
                    }
                  ></Form.Control>
                </Form.Group>
              </Col>
              <Col>
                <Form.Group>
                  ...
                  <Form.Select
                    value={repeatingTransfer.repeat}
                    onChange={(e) =>
                      updateRepeatedTransfer(
                        index,
                        "repeat",
                        e.target.value as Repeat,
                      )
                    }
                  >
              ...
              <Col>
                <Form.Group>
                  ...
                  <Form.Control
                    type="number"
                    value={repeatingTransfer.repeat_count}
                    onChange={(e) =>
                      updateRepeatedTransfer(
                        index,
                        "repeat_count",
              ...
              <Col>
                <Form.Group>
                  ...
                  <Form.Select
                    value={repeatingTransfer.from_account_id}
                    onChange={(e) =>
                      updateRepeatedTransfer(
                        index,
                        "from_account_id",
              ...
              <Col>
                <Form.Group>
                  ...
                  {repeatingTransfer.to_account_ids.map(
                    (accountId, toAccountIndex) => (
                      <div>
                        <Form.Select
                          value={accountId}
                          onChange={(e) =>
                            updateRepeatedTransfer(
                              index,
                              "to_account_ids",
                              repeatingTransfer.to_account_ids.map((value, i) =>
                                i === toAccountIndex ? e.target.value : value,
                              ),
                            )
                          }
                        >
                        ...
                        {repeatingTransfer.to_account_ids.length ===
                        1 ? null : (
                          <Button
                            variant="danger"
                            onClick={(_) => {
                              updateRepeatedTransfer(
                                index,
                                "to_account_ids",
                                repeatingTransfer.to_account_ids.filter(
                                  (_, i) => toAccountIndex !== i,
                                ),
                              )
                            }}
                          >
                    ...
                    <Button
                      ...
                      onClick={(_) => {
                        updateRepeatedTransfer(
                          index,
                          "to_account_ids",
                          repeatingTransfer.to_account_ids.concat(
                            defaultAccountId(settings, accounts),
                          ),
                        )
                      }}
                ...
                <Form.Group>
                  ...
                  <div>
                    <Button
                      ...
                      onClick={(_) => {
                        setUpdatedRepeatingTransfers(
                          updatedRepeatingTransfers.filter(
                            (_, i) => i !== index,
                          ),
                        )
                        setDirtyRepeatedTransfers(true)
          ...
          <div>
            <Button
              ...
              onClick={(_) => {
                setUpdatedRepeatingTransfers(
                  updatedRepeatingTransfers.concat({
                    start: today(),
                    repeat: Repeat.DAILY,
                    repeat_count: 1,
