...
export function BalanceTransactionModal({
  ...
  const mappedTransactions = filterTransactions(
    transactions,
    TransactionType.BALANCE,
    (transaction) => new Date(transaction.date) <= actualDate,
  ).reduce<{ [accountId: string]: JTranscation }>((acc, current) => {
        ...
        {Object.values(accounts)
          ...
          .map((id) => {
            return (
              <Form.Group key={id}>
                <Form.Label>
                  <AccountName accountId={id} accounts={accounts} />
                </Form.Label>
                <InputGroup>
                  <Form.Control
                    type="text"
                    ...
                    isInvalid={value(id) !== "" && !isValueValid(value(id))}
                    onChange={(e) => editTransaction(id, e.target.value)}
                  ></Form.Control>
                  <Button
                    onClick={() =>
                      editTransaction(id, "" + mappedTransactions[id]?.value)
                    }
                  >
                    <FontAwesomeIcon title="Use" icon={faFileImport} />
                  </Button>
                </InputGroup>
              </Form.Group>
            )
