            ...
            <Modal.Body>
                <Form>
                    ...
                    <Form.Group>
                        ...
                        <Form.Control type="date" disabled={specific} value={transactions?.date} onChange={e => setTransactions({ ...transactions, date: e.target.value })}></Form.Control>
                    </Form.Group>
                    <Form.Group>
                        <Form.Label>Type</Form.Label>
                        <Form.Select disabled={specific} value={transactions.type} onChange={e => {
                    ...
                    <Form.Group hidden={transactions.type !== TransactionType.TRANSFER}>
                        ...
                        <Form.Select disabled={transactions.type !== TransactionType.TRANSFER || specific} value={transactions.fromAccountId} onChange={e => setTransactions({ ...transactions, fromAccountId: e.target.value })}>
                            {
                        ...
                        return (
                            <React.Fragment key={index}>
                                ...
                                <Form.Group>
                                    ...
                                    <Form.Select disabled={specific} value={transaction?.accountId} onChange={e => editTransaction(index, { accountId: e.target.value })}>
                                        {
                                            Object.values(accounts)
                                                .filter(account => !(settings.NO_REGULAR_BALANCE_ACCOUNTS?.value ?? "").includes(account.id))
                                                .map(account => <option key={account.id} value={account.id}>{account.name} ({titleCase(account.type)})</option>)
