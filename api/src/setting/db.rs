    ...
    for setting in settings {
        if setting.value.contains(account_id.as_str()) {
            match setting.key {
                ...
                SettingKey::NoRegularBalanceAccounts | SettingKey::HiddenTransactionAccounts => {
...
fn verify(transaction: &Transaction, setting_key: SettingKey, value: String) -> anyhow::Result<()> {
    match setting_key {
        ...
        SettingKey::NoRegularBalanceAccounts | SettingKey::HiddenTransactionAccounts => verify_account_ids_exist_in_account_id_array(transaction, value),
