...
const EXPENSE_COLUMNS: &str = "id, description, category, date, value";
...
pub fn create_expense(transaction: &Transaction, new_expense: NewExpense) -> anyhow::Result<Option<Expense>> {
    return single(
        transaction,
        formatcp!("INSERT INTO expense ({EXPENSE_COLUMNS}) VALUES (?1, ?2, ?3, ?4, ?5) {EXPENSE_RETURNING}"),
        [Uuid::new_v4().to_string(), new_expense.description, new_expense.category.to_string(), new_expense.date.to_string(), normalize_decimal(&new_expense.value).to_string()],
    );
}
...
pub fn update_expense(transaction: &Transaction, updated_expense: Expense) -> anyhow::Result<Option<Expense>> {
    return single(
        transaction,
        formatcp!("UPDATE expense SET description = ?1, category = ?2, date = ?3, value = ?4 WHERE id = ?5 {EXPENSE_RETURNING}"),
        [updated_expense.description, updated_expense.category.to_string(), updated_expense.date.to_string(), normalize_decimal(&updated_expense.value).to_string(), updated_expense.id],
    );
}
